...
// API routes
// REMOVE if ./src/routes/api.routes.js does not exist (it causes a crash)
...
app.get('/events', (req, res) => res.render('pages/events', { title: 'Events' }));
...
function start(port, triesLeft = 5) {
  const server = app.listen(port, () => {
    console.log(`BillEasy server running at http://localhost:${port}`);
  });
  server.on('error', (err) => {
